...
public class transac extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        while (res.moveToNext()) {
            list.add(new evenement(res.getString(1),res.getString(3),res.getString(2)));
        }
        list.add(new evenement(nom,typeTrans, montTrans));


        final ListView listView = findViewById(R.id.myLV);
        listView.setAdapter(new affichage(this, list));
        this.pay = findViewById(R.id.butPay);

        Button add = findViewById(R.id.button);
        add.setOnClickListener(popupView -> {
            Intent addTransac = new Intent(getApplicationContext(), addTransaction.class);
            startActivity(addTransac);
            finish();
        });

        listView.setOnItemClickListener((a, v, position, id) -> {
            LayoutInflater inflater = (LayoutInflater) transac.this.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            View popupView = inflater.inflate(R.layout.creditcard, null, false);
